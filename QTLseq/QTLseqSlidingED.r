...
colnames(data)=c('chr','pos','ED','EDu')
...
head='#Chr\tPos\tED\twindSize\tCount\tStart\tEnd'
...
for (nn in 1:chrnum){
  ...
  for (ss in 1:steps){
    ...
    chrposdata_mean <- apply(chrposdata[4], 2, mean)
...
#    write(newdata, file = outfile, append = T, ncolumns = myncol + 10,sep = "\t")
    ...
    if(mincount > 0){
      if(chrposdata_cnt < mincount){
        chrposdata_null <- NaN
        newdata <- c(chrname[nn], pos0, chrposdata_null,actwidth, chrposdata_cnt,pos1,pos2)
      }else{
